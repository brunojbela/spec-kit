...
export function update({ dir, force = false, check = false, harnesses }) {
  const stale = isStale(dir);
  const stamp = readStamp(dir);
  if (!stamp) throw new Error('sem .spec-kit/installed.json — rode spec-kit init/analyze primeiro');
  ...
  // 1. fonte → catálogo (re-materializa agents/skills do pack)
...
export async function run(opts) {
  ...
  return update({ dir: opts.dir, force: opts.force, check: opts.check, harnesses: harnesses.length ? harnesses : undefined });
}
