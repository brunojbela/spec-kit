...
export async function run(opts) {
  ...
  if (!harnesses.length && !opts.yes && process.stdin.isTTY) {
    ...
    harnesses = await selectHarnesses({ message: 'Em quais harnesses instalar o squad (analyze)?' });
  }
  return analyze({ dir: opts.dir ?? opts.repo, stack: Array.isArray(opts.stack) ? opts.stack : (opts.stack ?? '').split(',').map((x)=>x.trim()).filter(Boolean), harnesses: harnesses.length ? harnesses : undefined });
}
