...
import { layoutOf } from '../harnesses/registry.js';
...
export function scaffoldProject({ dir, stack, goal, mode = 'greenfield', harnesses }) {
  ...
  const ctx = { stack, conventions: ['clean architecture', 'eslint', 'código pouco verboso'], structure: ['src', 'tests', 'docs'], gates: ['security-gate', 'docs-check', 'ledger-record'], mode };
...
export async function run(opts) {
  ...
  return scaffoldProject({ dir: opts.dir, stack: (opts.stack ?? '').split(',').filter(Boolean), goal: undefined, mode: 'greenfield', harnesses: harnesses.length ? harnesses : undefined });
}
