...
export function scaffoldProject({ dir, stack, goal, mode = 'greenfield', harnesses }) {
  if (!stack?.length) throw new Error('stack vazio — rode em terminal interativo ou passe --stack (ex: --stack laravel,react)');
...
export async function run(opts) {
  let stack = Array.isArray(opts.stack) ? opts.stack : (opts.stack ?? '').split(',').map((s) => s.trim()).filter(Boolean);
  let goal = opts.goal;
  // Mini-entrevista do PO (interativo): stack e objetivo antes dos harnesses.
  if ((!stack.length || !goal) && !opts.yes && process.stdin.isTTY) {
    const { ask } = await import('../prompt.js');
    if (!stack.length) stack = (await ask('Stack do projeto (ex: laravel,react): ')).split(',').map((s) => s.trim()).filter(Boolean);
    if (!goal) goal = await ask('Objetivo do projeto em 1 linha (opcional): ') || undefined;
  }
  ...
  return scaffoldProject({ dir: opts.dir, stack, goal, mode: 'greenfield', harnesses: harnesses.length ? harnesses : undefined });
}
